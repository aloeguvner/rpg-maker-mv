...
//=============================================================================
/*:
 * @plugindesc v1.0.0 Allows the developer to add icons to any command
 * in a command window.
 * @author Aloe Guvner
 * 
 * @param Icon Settings
...
Object.keys(PluginManager.parameters("ALOE_CommandIcons")).forEach(function (a) {
    return ALOE.CommandIconsParams[a] = Utils.recursiveParse(PluginManager.parameters("ALOE_CommandIcons")[a]);
});
...
Window_Command.prototype.drawCommandIcon = function (index, rect) {
    ...
    var wins = ALOE.CommandIconsParams.commandWindows;
    var wIndex = wins.findIndex(function (win) {
        return win.windowName === _this.constructor.name;
    });
    var cIndex = this.findCommandParameterIndex(index, wins, wIndex);
    if (cIndex >= 0) {
        var iconSize = ALOE.CommandIconsParams.iconSize;
        var iIndex = this.findCommandIconIndex(wins, wIndex, cIndex, index);
...
Window_Command.prototype.findCommandParameterIndex = function (index, wins, wIndex) {
    ...
    if (wIndex >= 0) {
        ...
        cIndex = wins[wIndex].iconCommands.findIndex(function (c) {
            return c.commandName.toLowerCase() === commandName.toLowerCase();
        });
    }
    return cIndex;
};
...
Window_ActorCommand.prototype.findCommandIconIndex = function (wins, wIndex, cIndex, index) {
    if (ALOE.CommandIconsParams.drawEquippedWeapon && this.commandName(index) === TextManager.attack) {
...
if (ALOE.CommandIconsParams.debugMode) {
    (function () {
        var Scene_Base_addWindow = Scene_Base.prototype.addWindow;
        ...
        Window_Command.prototype.logCommandInfo = function () {
            var logObject = {};
            logObject[this.constructor.name] = this._list.map(function (command) {
                return command.name;
            });
