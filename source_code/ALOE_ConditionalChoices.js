...
(function () {
        ...
        return choices
            .filter(choice => {
                ...
                if (match && match[1]) {
                    ...
                    if (match[1][0]) {
                        try {
                            const visible = !eval(match[1][0]);
                            if (visible) {
                                this._visibleChoices.push(choice);
                            }
                            return visible;
                        } catch (error) {
    ...
    Game_Message.prototype.updateVisibleIndexes = function (choices) {
        this._visibleChoices.forEach((visibleChoice, index) => {
            this._visibleChoiceIndexes[index] = choices.indexOf(visibleChoice);
        });
    };
    ...
    Game_Interpreter.prototype.setupChoices = function (params) {
        if (Imported.YEP_SelfSwVar) {$gameTemp.setSelfSwVarEvent(this._mapId, this._eventId);}
    ...
    const Game_Message_clear = Game_Message.prototype.clear;
